...
exp_mu = (force * 2 * np.pi * height * 1e-3) / (supply_current**2 * l)  # Experimental permeability
...
def per_current():
    ...
    for sc in unique_supply_currents:
        ...
        x = np.linspace(min(height[mask] -0.2), max(height[mask]), 100)
...
def force_vs_height():
    plt.figure(figsize=(8,6))
    plt.scatter(height, force, c=supply_current,  label='Data Points')
    plt.xlabel('Height (mm)')
    plt.ylabel('Force (N)')
    plt.title('Force vs Height for All Supply Currents')
    plt.grid(True)
    plt.legend()
    plt.show()
...
def force_vs_current():
    plt.figure(figsize=(8,6))
    plt.scatter(supply_current, force, label='Force vs Supply Current')
    x = np.linspace(min(supply_current), max(supply_current), 100)
    y = (mu_0 * x**2 * l) / (2 * np.pi * height.mean() * 1e-3)  # Force in Newtons
    plt.plot(x, y, 'r-', label='Ideal Line')
    plt.xlabel('Supply Current (A)')
    plt.ylabel('Force (N)')
    plt.title('Force vs Supply Current')
    plt.grid(True)
    plt.legend()
    plt.show()
...
def mu_vs_current():
    plt.figure(figsize=(8,6))
    plt.scatter(supply_current, exp_mu, label='Experimental Permeability')
    plt.axhline(y=mu_0, color='r', linestyle='--', label='Theoretical Permeability (H/m)')
    plt.xlabel('Supply Current (A)')
    plt.ylabel('Experimental Permeability (H/m)')
    plt.title('Experimental Permeability vs Supply Current')
    plt.grid(True)
    plt.legend()
    plt.show()